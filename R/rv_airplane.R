...
rv_create_airplane <- function(risk_ratio = 0.1,
                               ...
                               coord_ratio = 0.8) {
  seats <- rv_create_airplane_seats(
    rows = rows,
  ...
  if (isFALSE(compact)) {
    ...
    airplane_gg <- rv_create_airplane_compact(
      risk_ratio = risk_ratio,
...
rv_create_airplane_animation <- function(risk_ratio = c(`Scenario A` = 0.01, `Scenario B` = 0.1, `Scenario C` = 0.3, `Scenario D` = 0.7),
  ...
  if (is.data.frame(risk_ratio)) {
    seats_combo <- purrr::map2_dfr(
      .x = risk_ratio[["Ratio"]], .y = risk_ratio[["Risk"]],
      .f = function(x, y) {
        dplyr::bind_cols(
          seats,
          tibble::tibble(
            risk_places = rv_create_risk_vector(
              risk_ratio = x,
              total_places = total_places
            ),
            scenario = y
    ...
    seats_combo <- purrr::map2_dfr(
      .x = risk_ratio, .y = names(risk_ratio),
      .f = function(x, y) {
        dplyr::bind_cols(
          seats,
          tibble::tibble(
  ...
  gg_airplane_animated <- rv_create_airplane_graph_compact(
    seats = seats_combo,
    fill = fill,
    na_colour = na_colour,
    title = title,
